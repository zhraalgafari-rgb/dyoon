...
export function DesktopSidebar({ isOpen }: Props) {
  ...
  return (
    ...
    >
      {/* Logo Section */}
      <div className="relative p-4 md:p-4 pb-3 border-b border-border/40">
        <div className="flex items-center gap-3">
          <div className="relative size-10 md:size-10 rounded-xl bg-gradient-hero text-white flex items-center justify-center shadow-elevated shrink-0 transition-transform hover:scale-105 duration-300">
            <Wallet className="size-5" />
            <div className="absolute -top-1 -right-1 size-2.5 rounded-full bg-success animate-pulse-ring" />
          </div>
          <div className="min-w-0">
            <div className="font-black text-lg md:text-lg text-foreground leading-none tracking-tight">
              دفترك
            </div>
      ...
      {quickStats && (
        <div className="px-3 md:px-4 pt-3 pb-1">
          <div className="grid grid-cols-2 gap-2 p-3 rounded-xl bg-gradient-to-br from-background/80 to-background/40 border border-border/40">
            <div className="flex flex-col items-center text-center">
              <div className="flex items-center gap-1.5 text-xs font-bold text-success mb-1">
                <TrendingUp className="size-3.5" />
                له
              </div>
              <div className="text-sm font-black text-success tabular-nums leading-none">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="text-sm font-black text-danger tabular-nums leading-none">
                {new Intl.NumberFormat("en-US").format(quickStats.totalDebit)}
              </div>
            </div>
            <div className="col-span-2 pt-2 mt-2 border-t border-border/30 flex items-center justify-center gap-2">
          ...
          return (
            <div key={group.title}>
              <div className="flex items-center gap-2 px-2 mb-2">
                ...
                />
                <span
                  className={`text-xs font-black uppercase tracking-wider transition-colors ${activeGroup ? "text-primary/80" : "text-muted-foreground/60"
                    }`}
                >
                  {group.title}
                </span>
                ...
                {group.items.map((it) => {
                  ...
                  return (
                    <Link
                      ...
                      to={it.to}
                      preload="viewport"
                      className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${active
      ...
      <div className="border-t border-border/40 p-3">
        <div className="group relative flex items-center gap-3 rounded-xl bg-gradient-to-br from-secondary/60 to-secondary/30 px-3 py-2.5 transition-all hover:shadow-sm">
          {/* Avatar with ring */}
          <div className="relative shrink-0">
            <div className="size-9 rounded-full bg-gradient-primary text-primary-foreground flex items-center justify-center font-black text-sm shadow-glow transition-transform group-hover:scale-110 duration-300">
              {initial}
            </div>
